...
# set the base url
response <- read_html('https://en.wikipedia.org/wiki/Last_meal')
...
```{r}
foodbase <- read_xml('Foodbase/FoodBase_uncurated.xml')
...
name.groups <- parsed.ngrams %>%
  filter(!(word %in% c("meal", "food", "snack"))) %>% 
...
deduped.ngrams <- deduped.ngrams %>%
  mutate(S.word = if_else(substr(word,
                          ...
                          substr(word,
                                 ...
                                 stop = nchar(word)-1),
                          "NA"),
         S.match = S.word %in% word,
         word = if_else(S.match,
                        S.word,
                        word)) %>% 
  select(-c("S.word", "S.match"))
...
# still have issue of duplicates when it comes to adjectives 
```{r}
deduped.ngrams %>% filter(str_detect(word, 'ice cream')) %>% View
```
