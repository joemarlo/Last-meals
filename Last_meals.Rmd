...
# stemming
uses simple porter stemmer
others available: https://cran.r-project.org/web/packages/corpus/vignettes/stemmer.html
...
parsed.ngrams %>% 
  ...
  geom_col() +
  scale_fill_gradient(low = "#0b2919", high = "#2b7551") +
  labs(title = "Most common food in last meal requests",
       x = NULL,
       y = "Count") +
  coord_flip() +
  theme(legend.position = "none")
...
```{r}
# save 20  different graphs so we can make a gif in GIMP
set.seed(25)
for (i in 1:20){
  ...
  ggsave(filename = paste0("Plots/graph-gif-images/last_meals_graph", i, ".png"),
         plot = last_plot(),
         device = "png",
         width = 8,
         height = 7)
